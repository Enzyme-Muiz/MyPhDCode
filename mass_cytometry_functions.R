...
generate_concat <- function(y, x, breadth, column_needed, rename_columns_needed, transform = "yes", name_to_save_concat = "concatuntransformed") {
  if (!is.null(y)) {
    jfcs_files <- list.files(path = x, full.names = TRUE)
    sample_numbers <- c()
    for (j in y) {
